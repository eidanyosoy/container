
## 2022-03-26
 * [UPSTREAM] (github-actions[bot])
 * Delete description (doob187)
 * docs(README): update contributors (doob187)
 * [UPSTREAM] (github-actions[bot])
 * [Auto Generation] Changes : .gitignore .templates/apps/docker-auto-replyarr.sh .templates/apps/docker-backup.sh .templates/apps/docker-bazarr.sh .templates/apps/docker-cloudflared.sh .templates/apps/docker-deluge.sh .templates/apps/docker-dim.sh .templates/apps/docker-dockserverupdate.sh .templates/apps/docker-dockupdate.sh .templates/apps/docker-duplicati.sh .templates/apps/docker-emby.sh .templates/apps/docker-embystat.sh .templates/apps/docker-fenrus.sh .templates/apps/docker-gdsa.sh .templates/apps/docker-jackett.sh .templates/apps/docker-jellyfin.sh .templates/apps/docker-librespeed.sh .templates/apps/docker-lidarr.sh .templates/apps/docker-mount.sh .templates/apps/docker-multitautulli.sh .templates/apps/docker-overseerr.sh .templates/apps/docker-plex.sh .templates/apps/docker-prowlarr.sh .templates/apps/docker-qbittorrent.sh .templates/apps/docker-radarr.sh .templates/apps/docker-rclone.sh .templates/apps/docker-readarr.sh .templates/apps/docker-rutorrent.sh .templates/apps/docker-sabnzbd.sh .templates/apps/docker-sonarr.sh .templates/apps/docker-spotweb.sh .templates/apps/docker-tautulli.sh .templates/apps/docker-uploader.sh .templates/apps/docker-wiki.sh .templates/base/alpine.sh .templates/base/docker-alpine-v3.sh .templates/base/docker-dockserver.sh .templates/base/docker-haproxy.sh .templates/ci/Notes#CI .templates/ci/changes.sh .templates/ci/crlf.sh .templates/ci/dockserver.txt .templates/ci/gitlog.sh .templates/ci/move.sh .templates/ci/packages.sh .templates/ci/pipeline.sh .templates/ci/repack.sh .templates/ci/s6-overlay .templates/ci/update-packages.sh .templates/ci/version.sh .templates/dockserver.txt .templates/headline.txt .templates/nightly/docker-overseerr-nightly.sh .templates/nightly/docker-radarr-nightly.sh .templates/nightly/docker-sabnzbd-nightly.sh .templates/nightly/docker-sonarr-nightly.sh 11 (dockserver-bot[bot])
 * Update docker-spotweb.sh (doob187)
 * [Auto Generation] Changes : .templates/apps/docker-spotweb.sh (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update docker-spotweb.sh (doob187)
 * [Auto Generation] Changes : .templates/apps/docker-spotweb.sh (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)

## 2022-03-27
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Changes Packages] Changes : wiki/docs/install/container-gitlog.md (github-actions[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update rebase.yml (doob187)
 * [Auto Generation] Changes : .github/workflows/rebase.yml (dockserver-bot[bot])
 * [Auto Changes Packages] Changes : wiki/docs/install/container-gitlog.md (github-actions[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update headline.txt (doob187)
 * [Auto Generation] Changes : .templates/headline.txt (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)

## 2022-03-28
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update headline.txt (doob187)
 * [Auto Generation] Changes : .templates/headline.txt (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Changes Packages] Changes : wiki/docs/install/container-gitlog.md (github-actions[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update docker-radarr-nightly.sh (doob187)
 * Update docker-sonarr-nightly.sh (doob187)
 * Update docker-sabnzbd-nightly.sh (doob187)
 * Update docker-radarr.sh (doob187)
 * Update docker-sonarr.sh (doob187)
 * Update docker-sonarr-nightly.sh (doob187)
 * [Auto Generation] Changes : .templates/nightly/docker-sonarr-nightly.sh (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update docker-radarr.sh (doob187)
 * [Auto Generation] Changes : .templates/apps/docker-radarr.sh (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update release.yaml (doob187)
 * Update Dockerfile (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update headline.txt (doob187)
 * [Auto Generation] Changes : .templates/headline.txt (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update function.sh (doob187)
 * Update Dockerfile (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update docker-auto-replyarr.sh (doob187)
 * Update docker-backup.sh (doob187)
 * Update docker-bazarr.sh (doob187)
 * Update docker-cloudflared.sh (doob187)
 * Update docker-deluge.sh (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update Dockerfile (doob187)
 * Update docker-dockserverupdate.sh (doob187)
 * Update docker-dockupdate.sh (doob187)
 * Update docker-duplicati.sh (doob187)
 * Update docker-emby.sh (doob187)
 * Update docker-embystat.sh (doob187)
 * Update docker-fenrus.sh (doob187)
 * Update docker-gdsa.sh (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update docker-jackett.sh (doob187)
 * Update docker-jellyfin.sh (doob187)
 * Update docker-librespeed.sh (doob187)
 * Update docker-lidarr.sh (doob187)
 * Update docker-mount.sh (doob187)
 * Update docker-multitautulli.sh (doob187)
 * Update docker-overseerr.sh (doob187)
 * Update docker-plex.sh (doob187)
 * Update docker-prowlarr.sh (doob187)
 * Update docker-qbittorrent.sh (doob187)
 * Update docker-radarr.sh (doob187)
 * Update docker-rclone.sh (doob187)
 * Update docker-sabnzbd.sh (doob187)
 * Update docker-spotweb.sh (doob187)
 * Update docker-tautulli.sh (doob187)
 * Update docker-uploader.sh (doob187)
 * Update docker-wiki.sh (doob187)
 * Update docker-readarr.sh (doob187)
 * Update docker-sonarr.sh (doob187)
 * Update alpine.sh (doob187)
 * Update docker-alpine-v3.sh (doob187)
 * Update docker-dockserver.sh (doob187)
 * Update docker-haproxy.sh (doob187)
 * Update docker-overseerr-nightly.sh (doob187)
 * Update docker-radarr-nightly.sh (doob187)
 * Update docker-sabnzbd-nightly.sh (doob187)
 * Update docker-sonarr-nightly.sh (doob187)
 * [Auto Generation] Changes : .templates/nightly/docker-sonarr-nightly.sh (dockserver-bot[bot])
 * Update headline.txt (doob187)
 * Update Dockerfile (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update Dockerfile (doob187)
 * Update Dockerfile (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update Dockerfile (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * Update release.yaml (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)

## 2022-03-29
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Changes Packages] Changes : wiki/docs/install/container-gitlog.md (github-actions[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Changes Packages] Changes : wiki/docs/install/container-gitlog.md (github-actions[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Create automerg.yml (doob187)
 * [Auto Generation] Changes : .github/workflows/automerg.yml (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : apps/docker-gdsa/Dockerfile apps/docker-gdsa/release.json container.json wiki/docs/install/container-gitlog.md wiki/docs/install/container.json (dockserver-bot[bot])

## 2022-03-30
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Changes Packages] Changes : wiki/docs/install/container-gitlog.md (github-actions[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Changes Packages] Changes : wiki/docs/install/container-gitlog.md (github-actions[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)

## 2022-03-31
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Changes Packages] Changes : wiki/docs/install/container-gitlog.md (github-actions[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Delete automerg.yml (doob187)
 * [Auto Generation] Changes : .github/workflows/automerg.yml (dockserver-bot[bot])
 * Update docker-bazarr.sh (doob187)
 * [Auto Generation] Changes : .templates/apps/docker-bazarr.sh (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update docker-bazarr.sh (doob187)
 * [Auto Generation] Changes : .templates/apps/docker-bazarr.sh (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update docker-bazarr.sh (doob187)
 * [Auto Generation] Changes : .templates/apps/docker-bazarr.sh (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update docker-bazarr.sh (doob187)
 * [Auto Generation] Changes : .templates/apps/docker-bazarr.sh (dockserver-bot[bot])
 * Update docker-bazarr.sh (doob187)
 * [Auto Generation] Changes : .templates/apps/docker-bazarr.sh (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])

## 2022-04-01
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Changes Packages] Changes : github-metrics.svg (github-actions[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])
 * [Auto Generation] Changes : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * [Auto Changes Packages] Changes : apps/docker-overseerr/Dockerfile apps/docker-overseerr/release.json container.json wiki/docs/install/container-gitlog.md wiki/docs/install/container.json (github-actions[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changes : github-metrics.svg (dockserver-bot[bot])

## 2022-04-02
 * Update schedule.yaml (doob187)
 * Update scanrepo.yml (doob187)
 * Update release.yaml (doob187)
 * Update rebase.yml (doob187)
 * Update update-packages.yml (doob187)
 * [Auto Generation] Changes : .github/workflows/update-packages.yml (dockserver-bot[bot])
 * [Auto Generation] Changes : apps/docker-jackett/Dockerfile apps/docker-jackett/release.json container.json wiki/docs/install/container-gitlog.md wiki/docs/install/container.json (dockserver-bot[bot])
 * [Auto Changes Packages] Changes : wiki/docs/install/container-gitlog.md (github-actions[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update version.sh (doob187)
 * Update gitlog.sh (doob187)
 * Update schedule.yaml (doob187)
 * [Auto Generation] Changelog : .github/workflows/schedule.yaml (dockserver-bot[bot])
 * Create docker-create.sh (doob187)
 * Update docker-create.sh (doob187)
 * Create docker-config.sh (doob187)
 * [Auto Generation] Changelog : .templates/base/docker-config.sh (dockserver-bot[bot])
 * Update start.sh (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changelog : github-metrics.svg (dockserver-bot[bot])
 * Add additional loops to bypass the api bans (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update start.sh (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update start.sh (doob187)
 * [Auto Generation] Changelog : base/docker-dockserver/root/start.sh (dockserver-bot[bot])
 * Update start.sh (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changelog : github-metrics.svg (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * [Auto Generation] Changelog : github-metrics.svg (dockserver-bot[bot])
 * Update start.sh (doob187)
 * Update schedule.yaml (doob187)
 * [Auto Generation] Changelog : .github/workflows/schedule.yaml (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (doob187)
 * Update schedule.yaml (doob187)
 * [Auto Generation] Changelog : .github/workflows/schedule.yaml (dockserver-bot[bot])
 * [Auto Generation] Changelog : wiki/docs/install/container-gitlog.md (dockserver-bot[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (FSCorrupt)
 * [Auto Generation] Changelog : github-metrics.svg (dockserver-bot[bot])
 * docs(README): update contributors (FSCorrupt)
 * Delete rebase.yml (doob187)
 * Update schedule.yaml (doob187)
 * Delete intemperate.yml (doob187)
 * Delete scanrepo.yml (doob187)
 * Delete stale.yml (doob187)
 * Update github-metrics.svg - [Skip GitHub Action] (FSCorrupt)
 * Update version.sh (doob187)
 * Update version.sh (doob187)
 * Update schedule.yaml (doob187)
 * Update release.yaml (doob187)
 * [Auto Generation] Changelog : .github/workflows/release.yaml (dockserver-bot[bot])
 * [Auto Changes Packages] Changes : wiki/docs/install/container-gitlog.md (github-actions[bot])
 * Update github-metrics.svg - [Skip GitHub Action] (FSCorrupt)
