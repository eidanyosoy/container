...
NEWVERSION=$(curl -u $USERNAME:$TOKEN -sX GET "https://api.github.com/repos/sct/overseerr/releases/latest" | jq --raw-output '.tag_name')
...
PACKAGES="yarn"
VIRTUEL="--virtual=build-dependencies curl g++ jq curl wget tar make python3"
...
RUN \
  ...
  echo "'"**** install '"${APP}"' ****"'" && \
    ...
    curl -fsSL "'"https://github.com/sct/overseerr/archive/refs/tags/v"'${VERSION}'".tar.gz"'" | tar xzf - -C /app/overseerr --strip-components=1 && \
    '"${YARN}"' && \
    '"${LEFTOVER}"' && \
